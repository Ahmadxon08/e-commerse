        ...
        {cartItems.length > 0 ? (
          cartItems.map((item:CartType ) => (
            <div className="flex w-[50%] border-b gap-2 " key={item.id}>
              <img   src={item.thumbnail} alt="this is cart" className="flex w-[100px] h-[120px]" />
             ...
             <button onClick={()=>handleRemoveFromCart(item.id)} className='flex bg-red-400 px-2 rounded items-end relative right-0 justify-end'>
              Delete
             </button>
             </div>
            </div>
